...
부분 배낭 문제에 대해 알아보다가 배낭 문제라는 것은 조금 다르다는 것을 알게 되었고, 어떤 차이점이 있는지 알아보았다.
...
부분을 지우면 해결될 줄 알았다. 하지만 이 방법에는 문제가 있었다.
...
배낭 문제와 부분 배낭 문제는 어떤 때 의미가 있는 것일까?
두 문제를 실제 상황에 대입해 본다면 알 수 있는데, 배낭 문제는 어떤 분할 불가능한, 형태가 있는 물체를 배낭에 담는 것이며(고체?) 부분 배낭 문제는 형태가 없는 물체를 배낭에 담는 것이라고 볼 수 있다.(액체와 기체?)

부분 배낭 문제에 배낭 문제 코드를 사용할 경우에는 당연히 최대 무게를 사용하지 못해 최적해를 찾지 못하고, 배낭 문제에 부분 배낭 문제 코드를 사용할 경우에는 최적해와 다른 결론을 낸다.
 그리디 알고리즘은 항상 최적해를 구하는 것이 아니기 때문에 언제 사용하면 안되는지 생각해 볼 필요가 있다. 배낭 문제와 같은 조합 최적화 문제를 만나게 되면 평상시에 최적의 효율을 내더라도 결론적으로 최적해를 찾지 못하게 되기 때문.

 배낭 문제 시리즈는 여러 바리에이션이 많다. 조건을 조금만 추가해도 문제의 양상이 크게 달라지기 때문에 어떤 방식을 사용해서 문제를 풀지 먼저 고민하고 문제를 풀어야 하겠다.
